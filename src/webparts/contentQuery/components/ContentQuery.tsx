...
> {
  ...
  constructor(props: IContentQueryProps, state: IContentQueryState) {
    ...
    window[this.nsReactContentQuery][this.nsExternalScripts] =
      window[this.nsReactContentQuery][this.nsExternalScripts] || {};
  ...
  private isLastExecutedCall(timeStamp: number) {
    return (
      this.onGoingAsyncCalls.length > 0 &&
      this.onGoingAsyncCalls.filter((t: number) => {
        return t > timeStamp;
      }).length == 0
    );
  }
        ...
        if (scriptUrls && index < scriptUrls.length) {
          SPComponentLoader.loadScript(scriptUrls[index++])
            ...
            .catch(error => {
              ...
              //   _this_.logSource,
              //   error,
              //   _this_.props.wpContext.serviceScope
              // );
              console.log(error);
              next();
            });
  ...
  private loadTemplateContext() {
    if (this.areMandatoryFieldsConfigured()) {
      ...
      this.props
        .onLoadTemplateContext(this.props.querySettings, currentCallTimeStamp)
        .then((templateContext: IContentQueryTemplateContext) => {
          ...
          this.loadTemplate()
            ...
            .catch((error: string) => {
              this.setState({
                ...
                processedTemplateResult: null,
                error: Text.format(
                  this.props.strings.errorLoadingTemplate,
                  error
                )
              });
            });
        ...
        .catch(error => {
          this.setState({
            loading: false,
            processedTemplateResult: null,
            error: Text.format(this.props.strings.errorLoadingQuery, error)
          });
        });
    } else {
      this.setState({
  ...
  private loadTemplate(): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
      this.props
        .onLoadTemplate(this.props.templateUrl)
        .then((templateContent: string) => {
  ...
  private processTemplate(
    ...
    templateContext: IContentQueryTemplateContext
  ) {
    try {
      ...
      if (this.onGoingAsyncCalls.length == 0) {
        this.setState({
          loading: false,
          processedTemplateResult: result,
          error: null
        });
      }
      ...
      this.setState({
        loading: false,
        processedTemplateResult: null,
        error: Text.format(this.props.strings.errorProcessingTemplate, error)
      });
    }
  }
  ...
  private executeExternalCallbacks(callbackName: string) {
    if (this.props.externalScripts) {
      ...
      for (let scriptUrl of this.props.externalScripts) {
        ...
        var scriptNamespace =
          ReactContentQuery[this.nsExternalScripts][namespaceSuffix];
  ...
  private generateNamespaceFromScriptUrl(scriptUrl: string): string {
    return scriptUrl
      .substring(scriptUrl.lastIndexOf("/") + 1)
      .replace(".js", "")
      .replace(/[^a-zA-Z0-9]/g, "");
  }
  ...
  public componentDidUpdate(
    prevProps: IContentQueryProps,
    prevState: IContentQueryState
  ): void {
  ...
  public render(): React.ReactElement<IContentQueryProps> {
    const loading = this.state.loading ? (
      <Spinner label={this.props.strings.loadingItems} />
    ) : (
      <div />
    );
    const error = this.state.error ? (
      <div className={styles.cqwpError}>{this.state.error}</div>
    ) : (
      <div />
    );
          ...
          !this.state.error && (
            <div className={styles.cqwpValidations}>
              ...
              <Checkbox
                ...
                checked={!isEmpty(this.props.siteUrl)}
              />
              <Checkbox
                label={strings.WebUrlFieldLabel}
                checked={!isEmpty(this.props.querySettings.webUrl)}
              />
              <Checkbox
                label={strings.ListTitleFieldLabel}
                checked={!isEmpty(this.props.querySettings.listId)}
              />
              <Checkbox
                label={strings.viewFieldsChecklistStrings.label}
                checked={!isEmpty(this.props.querySettings.viewFields)}
              />
              <Checkbox
                label={
                  strings.templateTextStrings.dialogButtonLabel +
                  " / " +
                  strings.TemplateUrlFieldLabel
                }
                checked={
                  !isEmpty(this.props.templateUrl) ||
                  !isEmpty(this.props.templateText)
                }
              />
            </div>
          )}
          ...
          !this.state.error && (
            <div
              dangerouslySetInnerHTML={this.createMarkup(
                this.state.processedTemplateResult
              )}
            ></div>
          )}
      </div>
