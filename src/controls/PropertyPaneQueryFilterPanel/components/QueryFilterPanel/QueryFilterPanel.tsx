...
export class QueryFilterPanel extends React.Component<
  IQueryFilterPanelProps,
  IQueryFilterPanelState
> {
  /*************************************************************************************
  ...
  public componentDidUpdate(
    ...
    prevState: IQueryFilterPanelState
  ): void {
    if (
  ...
  private onFilterChanged(filter: IQueryFilter): void {
    ...
    let oldFilter = this.state.filters.filter(i => {
      return i.index == filter.index;
    })[0];
    let oldFilterIndex = this.state.filters.indexOf(oldFilter);

    if (
      this.props.trimEmptyFiltersOnChange &&
      this.isFilterEmpty(oldFilter) &&
      this.isFilterEmpty(filter)
    ) {
    ...
    if (isWorthNotifyingParent) {
      let filters: IQueryFilter[] = this.props.trimEmptyFiltersOnChange
        ? this.state.filters.filter(f => {
            return !this.isFilterEmpty(f);
          })
        : this.state.filters;
    ...
    if (filter.value == null || isEmpty(filter.value.toString())) {
      ...
      if (isEmpty(filter.expression)) {
        ...
        if (!filter.me) {
          ...
          if (
            filter.operator != QueryFilterOperator.IsNull &&
            filter.operator != QueryFilterOperator.IsNotNull
          ) {
            isFilterEmpty = true;
          }
        }
  ...
  private onAddFilterClick(): void {
    ...
    let nextAvailableFilterIndex =
      this.state.filters[this.state.filters.length - 1].index + 1;
    let newFilter: IQueryFilter = {
      index: nextAvailableFilterIndex,
      field: null,
      operator: QueryFilterOperator.Eq,
      join: QueryFilterJoin.Or,
      value: ""
  ...
  public render() {
    const loading = this.state.loading ? (
      <Spinner label={this.props.strings.loadingFieldsLabel} />
    ) : (
      <div />
    );
    const error =
      this.state.error != null ? (
        <div className="ms-TextField-errorMessage ms-u-slideDownIn20">
          {Text.format(
            this.props.strings.loadingFieldsErrorLabel,
            this.state.error
          )}
        </div>
      ) : (
    ...
    const filters = this.state.filters.map((filter, index) => (
      <div className={styles.queryFilterPanelItem} key={index}>
        <QueryFilter
          ...
          onLoadTaxonomyPickerSuggestions={
            this.props.onLoadTaxonomyPickerSuggestions
          }
          onLoadPeoplePickerSuggestions={
            this.props.onLoadPeoplePickerSuggestions
          }
      ...
      <div className={styles.queryFilterPanel}>
        ...
        {!this.state.loading && (
          <Button
            ...
            disabled={this.props.disabled}
          >
            {this.props.strings.addFilterLabel}
          </Button>
        )}
